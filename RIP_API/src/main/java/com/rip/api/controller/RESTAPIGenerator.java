...
public class RESTAPIGenerator {
    ...
    public ResponseEntity<String> generateRESTAPI(@RequestBody String ripJsonString) {
        ...
        try {
            ...
            AbstractProcessBuilder.buildProcess(null, "java -jar " + configDetails.getGeneratorPath() + " generate -i " + configDetails.getInputJsonPath() + " -l javascript -o " + configDetails.getOutputClientProject() + " -s -c " + configDetails.getConfigJsonPath()); // generate client project
            ...
            if (configDetails.getTechnicalSpecification().getString("orm_framework").equals("hibernate")) {
                ...
                BasicHibernateConfigurations configData = new BasicHibernateConfigurations(configDetails.getTechnicalSpecification().getString("dbms"), configDetails.getDatabaseDesign().getString("database_name"), configDetails.getResourcesPath(), configDetails.getProjectUtilPath(), configDetails.getPackageName(), hibernateConfigData, configDetails.getDatabaseDesign());
